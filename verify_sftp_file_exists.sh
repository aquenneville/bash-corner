...
# min time * * * 
# bash monitor_sftp_file.sh -e uat -p /sub/path/ -f base_filename 
#
...
_sys_current_year=$(date +"%Y")
_sys_current_date=$(date +"%Y%m%d")
...
#
_is_valid_env=$(is_contained_in_list "${envlist}" ${_env})
[[ $? -ne 0 ]] && {
